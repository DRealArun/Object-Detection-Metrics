...
def threshold_generic_processing(gtFolder, gtFormat, gtCoordType, imgSize, detCoordType, detFormat, detFolder, \
    iouThreshold, showPlot, savePath, useMask, useGtBBoxMask, useDtBBoxMask, cropMask):
...
if __name__ == '__main__':
    ...
    reject_eval_directories = [x[0].split('mask')[0] for x in os.walk(rootFolder) if 'mask' in x[0]]
    ...
    for useGtBBoxMask, useDtBBoxMask in [[True, True], [False, True], [False, False]]:
        for eval_dir in eval_directories:
            ...
            # for iouThreshold in [0.3]:
                ...
                process = Process(target=threshold_generic_processing, args=(gtFolder, gtFormat, gtCoordType, imgSize, detCoordType, detFormat, detFolder, \
        iouThreshold, showPlot, savePath, args.useMask, useGtBBoxMask, useDtBBoxMask, args.cropMask,))
                processes.append(process)
                process.start()

            for process in processes:
                process.join()
            print("Multi-processing complete !!")
